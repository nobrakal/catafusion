...
let go x =
  ...
  in para go xs
\end{minted}
\noindent The \minline{map} happen for each suffix! We clearly not want that.
...
\begin{itemize}
\item We cannot target \minline{cata coerce}, since \minline{cata} is just \minline{para}. We cannot either target\\ \minline{para (coerce . fmap snd)}, since \minline{fmap} is a class function; it is consequently difficult to avoid unneeded destruction %todo such as...

\item It is not compatible with GHC's \minline{build}, as discussed in section \ref{sec:parabuild}.
\end{itemize}

We consequently choose to target a specific function that represent either a catamorphism or a paramorphism, called \minline{destruct}, defined with according rules in figure \ref{fig:para}.
%todo explain more
...
%TODO infinite buildR: There does not seems to be a valid way to handle this (and a very reduced interest)
