...
\end{minted}
\noindent How \hs{cata} is working? As any fold on any structure! Given a recursive structure, we will firstly destruct any substructure, and then applies a function to the result. The "firstly destruct any substructure" step is performed by using a \hs{Functor} instance: it is the only way to get "inside" the structure, since we don't know how it is made. The combination function can also feel a bit strange but its type says it all: if we have a recursive structure where the substructures were replaced by a result, we must be able to combine them to produce a result.
