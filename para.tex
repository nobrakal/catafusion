...
We can then define a rule that will only target paramorphisms. However, paramorphism fusion is not as classy as the catamorphisms one: it can produce a lot of code and is not very compatible with GHC's rules for lists TODO (: problèmes avec les listes, + cata coerce). We consequently choose to target a specific function that represent either a catamorphism or a paramorphism, called \minline{destruct} (defined in figure \ref{fig:para}), which will \emph{not} be visible to the end user.
