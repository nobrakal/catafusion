...
\label{subsec:defi}
First of all, classic recursive structure can be defined as \emph{fixed-points}, using the \minline{Fix} data-type:
...
A \emph{catamorphism} generalizes a fold on a recursive structure. For example, catamorphisms on lists can be expressed with the so-called \minline{foldr}.
\begin{minted}{Haskell}
foldr :: (a -> b -> b) -> b -> [a] -> b
foldr _ z []     = z
foldr k z (y:ys) = k y (foldr k z ys)
\end{minted}
The idea of \minline{foldr} is that, if you provide a definition for the empty list and another for the \verb|cons| operation, then you can go trough a list and destruct it recursively. Note that we didn't use the concept of list but just its constructors; consequently you can use this process for any other recursive type. The definition of a type in term of \minline{Fix} allows a straightforward implementation of \minline{cata}, a high-order function for catamorphisms.
...
\noindent There is a better implementation of \minline{cata} using the same definitions as E. Kmett in his \verb|recusrion-schemes| package \cite{ekmett:eschems}:
...
\noindent \minline{unfoldr} takes a function that, from a starting seed, produces maybe an element and another seed, or nothing. As clever reader already guessed, we can provide anamorphisms for any fixed-point structure, using:
\begin{minted}{Haskell}
...
Hylomorphisms can be expressed using \minline{hylo} (as in the \verb|recursion-schemes| package \cite{ekmett:eschems}):
\begin{minted}{Haskell}
hylo :: Functor f =>
  (f b -> b) -> (a -> f a) -> a -> b
