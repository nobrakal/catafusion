...
Let us take back our \minline{has2leaves} example. If we want to have \minline{hasLeaf} fused with a possible previous operation on the tree (like a bind), we need to have not only \minline{bind} on a re-writeable form, but also need that the transformation of the combination function's arguments to be in a re-writeable form. TODO
\begin{figure}
\begin{minted}{Haskell}
